...
class RelasiAtasanController extends Controller
{
    public function __construct()
    {
        $this->judul = "RelasiAtasan";
        $this->allowedWhere = ['tanggal'];
        $this->selectFilter = [];
        $this->fulltext = '`nama`, `nohp`, `username`';
        $this->mainModel = 'app\Model\RelasiAtasan';
        $this->hapusHuruf = [];
        $this->formatUang = [];
        $this->except = ['id'];
    ...
    {
        $data = parent::SetIndex($Request, $Session);
        $data['selected_karyawan_id'] = !empty($Request->karyawan_id) ? (int) $Request->karyawan_id : null;
    ...
    {
        $data = [
            'judul' => 'Form Relasi Atasan',
            'path' => 'Pages.RelasiAtasan.ModalForm',
            'link' => 'RelasiAtasan-View',
            'icon' => 'fa fa-lock',
        ...
        if (!empty($Request->key)) {
            ...
            $data['path'] = 'Pages.RelasiAtasan.View';
        }
