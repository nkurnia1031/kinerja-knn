...
class LaporanKaryawanController extends Controller
{
    ...
    {
        $this->judul = "LaporanKaryawan";
        $this->allowedWhere = ['tanggal'];
        $this->selectFilter = [];
        $this->fulltext = '`nama`, `nohp`, `username`';
        $this->mainModel = 'app\Model\LaporanKaryawan';
        $this->hapusHuruf = [];
        $this->formatUang = [];
        $this->except = ['id'];
    ...
    public function View($Request, $Session, $blade)
    {
        $data = [
            'judul' => 'Form User',
            'path' => 'Pages.LaporanKaryawan.ModalForm',
            'link' => 'LaporanKaryawan-View',
            'icon' => 'fa fa-lock',
        ...
        if (!empty($Request->key)) {
            ...
            $data['path'] = 'Pages.LaporanKaryawan.View';
        }
